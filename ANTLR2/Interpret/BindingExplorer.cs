...
namespace ANTLR2.Interpret {
    class BindingExplorer : gramBaseVisitor<Tree<Binding>>{
        ...
        public BindingExplorer(ExprVisitor interpreter, IValue value) {
        ...
        public override Tree<Binding> VisitBinding_multiple(gramParser.Binding_multipleContext context) {
            var bindings = context.binding().Zip(value.Get<IEnumerable<IValue>>(), (binding, subValue) => new BindingExplorer(interpreter, subValue).Visit(binding));
